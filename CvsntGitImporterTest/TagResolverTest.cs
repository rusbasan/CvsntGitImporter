...
public class TagResolverTest
{
    ...
    public void Resolve_ReorderCommits_TwoFilesThatDontMove()
    {
        ...
        Assert.IsTrue(result, "Succeeded");
        Assert.IsTrue(resolver.Commits.Select(c => c.CommitId).SequenceEqual("c0", "c2", "c3", "c1"),
            "Commits reordered");
    ...
    public void Resolve_SplitCommit()
    {
        ...
        Assert.IsTrue(newCommits[2].Single().File.Name == "file2" &&
                      newCommits[2].Single().Revision.Equals(Revision.Create("1.2")));
        Assert.AreEqual(newCommits[3].CommitId, "c3");
        Assert.IsTrue(newCommits[4].Single().File.Name == "file1" &&
                      newCommits[3].Single().Revision.Equals(Revision.Create("1.3")));
    ...
    public void Resolve_SplitCommit_SplitCandidate()
    {
        ...
        Assert.IsTrue(newCommits[2].Single().File.Name == "file2" &&
                      newCommits[2].Single().Revision.Equals(Revision.Create("1.2")));
        Assert.IsTrue(newCommits[3].Single().File.Name == "file1" &&
                      newCommits[3].Single().Revision.Equals(Revision.Create("1.3")));
    }
    ...
    public void Resolve_ReorderWithCreatedAndModifiedFileInTheMiddle()
    {
        ...
        Assert.IsTrue(resolver.Commits.Select(c => c.CommitId).SequenceEqual("c0", "c3", "c1", "c2"),
            "Commits reordered");
        Assert.AreSame(resolver.ResolvedTags["tag"], target);
    }
    ...
    public void Resolve_FileCreatedOnTrunkAndBackported()
    {
        ...
        var commits = new List<Commit>()
        {
            ...
            new Commit("c2").WithRevision(file3, "1.1")
                .WithRevision(file1, "1.2"), // add file3 on trunk, but also take file1 to r1.2
