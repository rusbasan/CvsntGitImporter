...
public class ConfigTest
{
    ...
    public void BranchpointRule_Valid()
    {
        ...
        config.ParseCommandLineSwitches("--sandbox", Path.GetTempPath(), "--branchpoint-rule",
            @"^(.*)$/$1-branchpoint");
    ...
    public void IncludeFile_IncludesHeadOnly()
    {
        var switches = new Switches();
        var config = new Config(switches);
        config.ParseCommandLineSwitches("--sandbox", Path.GetTempPath(), "--head-only", "^file1", "--include",
            "^file2");
